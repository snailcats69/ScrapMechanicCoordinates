...
function SurvivalPlayer.cl_localPlayerUpdate( self, dt )
	...
	if g_survivalHud then
		...
		if character then
			...
			yaw = math.floor(math.deg(yaw))

			local text = math.floor(position.x).." "..math.floor(position.y).." | "..math.floor(position.x%cell_size).." "..math.floor(position.y%cell_size).." | "..yaw.."°"
			g_survivalHud:setText( "Time", text )
		end
	end
